/**
 * @file crypto.cpp
 * @brief Provides cryptographic utility functions for hashing, signing, and
 * verifying.
 *
...
namespace sbmpi
{
  namespace util
  {
     ...
     * @return An array of 32 unsigned char representing the generated private
     * key.
     */
      ...
      do {
        if (RAND_bytes(bytes.data(), bytes.size()) != 1) {
          throw std::runtime_error(
              "[CRYPTO]: Failed to generate secure private key!");
        }
     ...
     */
    std::vector<unsigned char> derivePublicKey(
        const std::vector<unsigned char>& privateKey)
    {
      ...
      // First bytes to denote uncompressed key, next 32 bytes for x-coord in
      // elliptic key cryptography, last 32 bytes for the y-coord.
      size_t                     publicKeyLen = (KEYLEN * 2) + 1;
      std::vector<unsigned char> publicKeySerialized(publicKeyLen);
      secp256k1_ec_pubkey_serialize(ctx, publicKeySerialized.data(),
     ...
     *
     * The address is the last 20 bytes of the Keccak-256 (SHA3-256) hash of the
     * public key (excluding the 0x04 prefix).
     * @param publicKey Serialized uncompressed public key (65 bytes, starting
     * with 0x04).
    ...
    std::string deriveAddress(const std::vector<unsigned char>& publicKey)
    {
      ...
      std::vector<unsigned char> pubSlice(hashBuffer + 1,
                                          hashBuffer + publicKey.size());
     ...
     * Given a valid private key was provided, this function constructs a
     * recoverable ECDSA signature using the computed Keccak-256 hash of
     * transaction data and a private key. The function returns the signature as
     * a vector containing unsigned chars representing hexadecimal bytes. The
     * first 64 bytes of the vector represent the signature, where the last byte
     * is the recovery ID used in public key recovery and verification.
     * @param data The data to be signed.
     * @param privateKey The private key used for signing.
     * @return A std::vector<unsigned char> representing the signature +
     * recovery ID in hexadecimal bytes.
     */
        ...
        const std::vector<unsigned char>& privateKey)
    {
      ...
      // Verify that the private key is a valid EC private key less than
      ...
      // Create a recoverable signature (essential for verifying) instance for
      // the initial signature
      secp256k1_ecdsa_recoverable_signature sig;
      if (!secp256k1_ecdsa_sign_recoverable(
              ctx, &sig, data.data(), privateKey.data(), nullptr, nullptr)) {
        secp256k1_context_destroy(ctx);
        throw std::runtime_error("[CRYPTO]: Failed to sign transaction");
      }

      // Now serialize the secp256k1 signature into bytes, and extract recovery
      // ID
      ...
      std::vector<unsigned char> signature(sigCompact,
                                           sigCompact + (KEYLEN * 2));
     ...
     *
     * Uses the secp256k1 library for ECDSA operations.
     * Given that valid data and signature bytes were provided, this function
     * starts by recovering the public key from the recoverable signature and
     * data. Then, the signature is parsed and converted to a suitable lower-S
     * format. Finally, the signature validity is returned using the secp256k1
                ...
                const std::vector<unsigned char>& signature)
    {
      ...
      if (signature.size() < (util::KEYLEN * 2) + 1 ||
          data.size() < util::KEYLEN) {
      ...
      // Since the signature parameter includes 65 bytes, extract the signature
      // (64 bytes), and the recovery ID (last byte)
      std::vector<unsigned char> signatureBytes(signature.begin(),
                                                signature.end() - 1);
      ...
      if (!secp256k1_ecdsa_recoverable_signature_parse_compact(
        ...
        secp256k1_context_destroy(ctx);
        throw std::runtime_error(
            "[CRYPTO]: Could not parse recoverable signature!");
      }

      // After creating the recoverable signature instance, recover the public
      // key used in verification
      secp256k1_pubkey publicKey;
      if (!secp256k1_ecdsa_recover(ctx, &publicKey, &recoverSig, data.data())) {
        secp256k1_context_destroy(ctx);
        throw std::runtime_error(
            "[CRYPTO]: Could not recover public key from signature!");
      }

      // A normal signature is required in the verify function, so convert
      // recoverable to normal
      secp256k1_ecdsa_signature normalSig;
      if (!secp256k1_ecdsa_recoverable_signature_convert(ctx, &normalSig,
                                                         &recoverSig)) {
        secp256k1_context_destroy(ctx);
        throw std::runtime_error(
            "[CRYPTO]: Could not convert recoverable signature!");
      }

      // The verify function also requires a normalize signature (lower-S
      // format)
      secp256k1_ecdsa_signature sigNormalized;
      secp256k1_ecdsa_signature_normalize(ctx, &sigNormalized, &normalSig);

      // Finally verify the parsed and converted signature for signature
      // validity with respect to the passed data.
      bool isValid =
          secp256k1_ecdsa_verify(ctx, &sigNormalized, data.data(), &publicKey);
      ...
      while (currentTransactions.size() != 1) {
        ...
        for (size_t i = 0; i < currentTransactions.size(); i += 2) {
          ...
          if (i + 1 < currentTransactions.size()) {
            ...
            hashBytes.insert(hashBytes.end(), currentTransactions[i].begin(),
                             currentTransactions[i].end());
            hashBytes.insert(hashBytes.end(),
                             currentTransactions[i + 1].begin(),
                             currentTransactions[i + 1].end());
