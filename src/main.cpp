...
int main(int argc, char* argv[])
{
  ...
  MPI_Bcast(&config, sizeof(sbmpi::util::ExperimentConfig), MPI_CHAR, 0,
            MPI_COMM_WORLD);

  sbmpi::util::Logger logger(
      world_rank, static_cast<sbmpi::util::LogLevel>(config.log_level));
  sbmpi::util::Timer timer;

  if (world_rank == 0) {
    logger.info("Simulation starting with " + std::to_string(world_size) +
                " total nodes.");
  ...
  int nodes_per_shard =
      (world_size - config.final_committee_size) / config.num_shards;
  if (nodes_per_shard < (3 * config.faulty_nodes_per_shard + 1)) {
    if (world_rank == 0) {
      logger.error("Not enough nodes per shard to tolerate " +
                   std::to_string(config.faulty_nodes_per_shard) +
                   " faults. Need at least " +
                   std::to_string(3 * config.faulty_nodes_per_shard + 1));
    }
    MPI_Finalize();
    return 1;
  }
  ...
  } else if (world_rank < config.num_shards * nodes_per_shard +
                              config.final_committee_size) {
  ...
  if (local_comm != MPI_COMM_NULL) {
    ...
    if (color < config.num_shards) {
      role = (local_rank == 0) ? sbmpi::core::NodeRole::SHARD_LEADER
                               : sbmpi::core::NodeRole::SHARD_MEMBER;
    } else {
      role = (local_rank == 0) ? sbmpi::core::NodeRole::FINAL_COMMITTEE_LEADER
                               : sbmpi::core::NodeRole::FINAL_COMMITTEE_MEMBER;
    }
  }
  ...
  if (world_rank == 0) {
    auto transactions =
        sbmpi::core::createMockTransactions(config.total_transactions);
    ...
    for (int i = 0; i < config.num_shards; ++i) {
      ...
      int tx_count           = (i == config.num_shards - 1)
                                   ? (transactions.size() - i * tx_per_shard)
                                   : tx_per_shard;

      logger.info("Sending " + std::to_string(tx_count) +
                  " transactions to shard leader " +
                  std::to_string(leader_global_rank));
      ...
      MPI_Send(tx_buffer.data(), tx_buffer.size(), MPI_CHAR, leader_global_rank,
               1, MPI_COMM_WORLD);
    }
  }

  // --- Run Simulation ---
  if (node.getRole() == sbmpi::core::NodeRole::SHARD_LEADER ||
      node.getRole() == sbmpi::core::NodeRole::SHARD_MEMBER) {
    ...
    shard.runMainLoop();
  } else if (node.getRole() == sbmpi::core::NodeRole::FINAL_COMMITTEE_LEADER ||
             node.getRole() == sbmpi::core::NodeRole::FINAL_COMMITTEE_MEMBER) {
  ...
  if (world_rank == 0) {
    ...
    std::cout << "----------------------------------------" << std::endl;
    std::cout << "Total Execution Time: " << duration << " seconds"
              << std::endl;
