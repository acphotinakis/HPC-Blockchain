...
void PBFT::prePrepare(const core::blocks::MicroBlock& block) {
  ...
  // transactions. Or we can broadcast the block first. Let's assume the leader
  // does that.
...
core::blocks::MicroBlock PBFT::run(
  ...
  if (myRank == leaderRank) {
    ...
    block.header =
        core::blocks::BlockHeader(1, "genesis_hash_placeholder", "merkle_placeholder");
    ...
    // For simplicity, let's assume a max block size or use a probe-recv pattern.
    // For now, let's assume the blockData is correctly sized by the bcast.
  ...
  if (myRank != leaderRank) {
    ...
    // This part needs to be more robust, handling multiple messages and timeouts.
    // For now, a simple receive from leader.
  ...
  // This is a simplified collection. In reality, it would involve non-blocking
  ...
  for (int i = 0; i < numNodes; ++i) {
    ...
    if (msg.type == PBFTMessageType::PREPARE && msg.blockHash == proposedBlockHash) {
      prepareCount++;
    }
  }
  ...
  for (int i = 0; i < numNodes; ++i) {
    ...
    std::vector<char> msgData = network::recv(i, 0, communicator);
    PBFTMessage msg = deserializeMessage(msgData);
    if (msg.type == PBFTMessageType::COMMIT && msg.blockHash == proposedBlockHash) {
      commitCount++;
    }
  }
  ...
  if (commitCount >= 2 * maxFaultyNodes + 1) {
    ...
    return core::blocks::MicroBlock(); // Return an empty block to indicate failure
  }
}
