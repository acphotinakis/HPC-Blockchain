...
def plot_simulation_metrics(column_name, group_name, graph_name):
    ...
    grouped = df.groupby(['NP', 'NumShards', 'NumTransactions'])[column_name].mean().reset_index()
    grouped = grouped.rename(columns={column_name: group_name})  # Change our total time column to average time
    ...
    for i, np_val in enumerate(unique_np):
        ...
        for config in unique_configs:
            ...
            if not match.empty:
                values.append(match[group_name].values[0])
            else:
                values.append(0)
    ...
    ax.set_title(f'{graph_name} by Configuration')
    ax.set_xticks(x_pos)
    ...
    plt.savefig(f'{group_name}_metrics_plot.png')
    plt.close()
