...
/**
 * @file transaction.h
 * @brief Defines the Transaction class, representing a single transaction in the blockchain.
 *
 * The Transaction class encapsulates all data related to a single transaction,
 * including sender and receiver addresses, the amount, and a cryptographic signature.
 * It provides methods for serialization and deserialization to enable network transport
 * via MPI.
 */
...
public:
    ...
    /**
     * @brief Serializes the Transaction object into a byte vector for network transmission.
     *
